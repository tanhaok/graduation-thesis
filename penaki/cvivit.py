...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        self.to_logits = nn.Sequential(
            ...
            Rearrange('b ... -> b (...)'), nn.Linear(latent_dim, 1),
            Rearrange('b 1 -> b'))
...
class CViViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.to_patch_emb = nn.Sequential(
            ...
            nn.LayerNorm(channels * patch_width * patch_height *
                         temporal_patch_size),
            nn.Linear(
                channels * patch_width * patch_height * temporal_patch_size,
                dim), nn.LayerNorm(dim))
        ...
        self.enc_spatial_transformer = Transformer(depth=spatial_depth,
                                                   **transformer_kwargs)
        self.enc_temporal_transformer = Transformer(depth=temporal_depth,
                                                    **transformer_kwargs)

        self.vq = VectorQuantize(dim=dim,
                                 codebook_size=codebook_size,
                                 use_cosine_sim=True)

        self.dec_spatial_transformer = Transformer(depth=spatial_depth,
                                                   **transformer_kwargs)
        self.dec_temporal_transformer = Transformer(depth=temporal_depth,
                                                    **transformer_kwargs)
        ...
        self.discr = Discriminator(image_size=self.image_size,
                                   dim=discr_base_dim,
                                   channels=channels,
    ...
    def calculate_video_token_mask(self, videos, video_frame_mask):
        *_, h, w = videos.shape
        ...
        first_frame_mask, rest_frame_mask = video_frame_mask[:, :
                                                             1], video_frame_mask[:,
                                                                                  1:]
        rest_vq_mask = rearrange(rest_frame_mask,
                                 'b (f p) -> b f p',
                                 p=self.temporal_patch_size)
        video_mask = torch.cat((first_frame_mask, rest_vq_mask.any(dim=-1)),
                               dim=-1)
    ...
    def num_tokens_per_frames(self, num_frames, include_first_frame=True):
        ...
        return total_tokens + int(
            num_frames / self.temporal_patch_size) * image_num_tokens
    ...
    def forward(self,
        ...
        *_, h, w, _ = shape
        ...
        returned_recon = rearrange(
            recon_video,
            'b c 1 h w -> b c h w') if is_image else recon_video.clone()
        ...
        if return_discr_loss:
            ...
            discr_loss = self.discr_loss(recon_video_discr_logits,
                                         video_discr_logits)
        ...
        if video.shape[1] == 1:
            input_vgg_input, recon_vgg_input = map(
