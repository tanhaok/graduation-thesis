...
def FeedForward(dim, mult=4, dropout=0.):
    ...
    return nn.Sequential(nn.LayerNorm(dim),
                         nn.Linear(dim, inner_dim * 2, bias=False), GEGLU(),
                         nn.Dropout(dropout),
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, context=None, attn_bias=None):
        ...
        nk, nv = repeat(self.null_kv, 'h (n r) d -> b h n r d', b=batch,
                        r=2).unbind(dim=-2)
        ...
        if self.causal:
            ...
            causal_mask = torch.ones((i, j), device=device,
...
class ContinuousPositionBias(nn.Module):
    ...
    def __init__(
        ...
        self.net.append(
            nn.Sequential(nn.Linear(self.num_dims, dim), leaky_relu()))
    ...
    def forward(self,
        ...
        for peg, self_attn, cross_attn, ff in self.layers:
            ...
            if exists(cross_attn) and exists(context):
                x = cross_attn(
                    x, context=context, mask=cross_attn_context_mask) + x
