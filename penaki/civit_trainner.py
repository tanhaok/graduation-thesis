...
class CViViTTrainer(nn.Module):
    ...
    def __init__(self,
        ...
        (self.vae, self.optim, self.discr_optim, self.dl,
         self.valid_dl) = self.accelerator.prepare(self.vae, self.optim,
                                                   self.discr_optim, self.dl,
                                                   self.valid_dl)
    ...
    def save(self, path):
        ...
        pkg = dict(model=self.accelerator.get_state_dict(self.vae),
    ...
    def is_distributed(self):
        return not (self.accelerator.distributed_type == DistributedType.NO
    ...
    def train_step(self):
        ...
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(self.vae.parameters(),
                                             self.max_grad_norm)
        ...
        if exists(self.vae.discr):
            ...
            for _ in range(self.grad_accum_every):
                ...
                accum_log(logs,
                          {'discr_loss': loss.item() / self.grad_accum_every})

            if exists(self.discr_max_grad_norm):
                self.accelerator.clip_grad_norm_(self.vae.discr.parameters(),
        ...
        if self.is_main and not (steps % self.save_results_every):
            ...
            if self.use_ema:
                vaes_to_evaluate = ((self.ema_vae.ema_model,
                                     f'{steps}.ema'), ) + vaes_to_evaluate
            ...
            for model, filename in vaes_to_evaluate:
                ...
                if is_video:
                    ...
                    for tensor in recons.unbind(dim=0):
                        video_tensor_to_gif(
                            tensor,
                            str(sampled_videos_path / f'{filename}.gif'))
                else:
                    imgs_and_recons = torch.stack((valid_data, recons), dim=0)
                    imgs_and_recons = rearrange(imgs_and_recons,
                                                'r b ... -> (b r) ...')

                    imgs_and_recons = imgs_and_recons.detach().cpu().float(
                    ).clamp(0., 1.)
                    grid = make_grid(imgs_and_recons,
                                     nrow=2,
                                     normalize=True,
                                     value_range=(0, 1))
                    ...
                    save_image(grid,
                               str(self.results_folder / f'{filename}.png'))
